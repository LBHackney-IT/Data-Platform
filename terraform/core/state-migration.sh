#./state-migration.sh development -dry-run 2>&1 | tee state-migration-output
rm -f state-migration-output
FILES="../etl/*.tf"
for f in $FILES
do
    grep -Eo -e '^module \".*\"' -e '^data \".*\"' "$f" | tr -s " \"" "." | sed 's/\.$//' | \
    while read -r resource_address; do aws-vault exec hackney-dataplatform-"${1}" -- terraform state mv $2 -state-out=./tfstate-etl.tfstate "$resource_address" "$resource_address"; done
