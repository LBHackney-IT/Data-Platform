locals {
  ...
  vonage_glue_trigger_name = local.is_live_environment ? module.copy_icaseworks_data_landing_to_raw[0].trigger_name : ""
}
...
module "copy_vonage_data_landing_to_raw" {
  ...
  job_parameters = {
    "--job-bookmark-option" = "job-bookmark-enable"
    "--s3_bucket_target"    = "${module.raw_zone.bucket_id}/customer-services"
    "--s3_bucket_source"    = module.landing_zone.bucket_id
    "--s3_prefix"           = "manual/vonage/"
    "--extra-py-files"      = "s3://${module.glue_scripts.bucket_id}/${aws_s3_bucket_object.helpers.key}"
  }
  crawler_details = {
