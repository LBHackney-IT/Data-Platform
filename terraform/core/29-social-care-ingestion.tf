...
locals {
  table_filter_expressions_social_care = local.is_live_environment ? {
    social_care_tables = "To update once known"
  } : {}
...
module "copy_social_care_adult_social_care_to_raw_zone" {
  ...
  job_parameters = {
    ...
    #    "--table_filter_expression"   = "" TODO: update with relevant table filter expression
    "--glue_database_name_source" = aws_glue_catalog_database.landing_zone_social_care.name
    "--glue_database_name_target" = module.department_adult_social_care.raw_zone_catalog_database_name
...
module "copy_social_care_children_family_services_to_raw_zone" {
  count = local.is_live_environment ? 1 : 0
  tags  = module.tags.values
  ...
  job_parameters = {
    ...
    #    "--table_filter_expression"          = "" TODO: update with relevant table filter expression
    "--glue_database_name_source"        = aws_glue_catalog_database.landing_zone_social_care.name
    "--glue_database_name_target"        = module.department_children_family_services.raw_zone_catalog_database_name
    "--enable-glue-datacatalog"          = "true"
