module "kafka_event_streaming" {
  ...
  source      = "../modules/resources/kafka-event-streaming"
  tags        = module.tags.values
  environment = var.environment
  project     = var.project

  glue_iam_role                          = data.aws_iam_role.glue_role.name
  glue_database_name                     = data.aws_glue_catalog_database.landing_zone_catalog_database.name
