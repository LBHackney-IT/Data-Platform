...
module "sagemaker" {
  source = "../modules/resources/sagemaker/"
  count  = local.is_live_environment ? 1 : 0

  development_endpoint_role_arn = data.aws_iam_role.glue_role.arn
  tags                          = module.tags.values
  identifier_prefix             = local.short_identifier_prefix
  python_libs                   = "s3://${module.glue_scripts.bucket_id}/${data.aws_s3_bucket_object.helpers.key},s3://${module.glue_scripts.bucket_id}/${data.aws_s3_bucket_object.pydeequ.key}"
  extra_jars                    = "s3://${module.glue_scripts.bucket_id}/${data.aws_s3_bucket_object.jars.key}"
  instance_name                 = "admin"
