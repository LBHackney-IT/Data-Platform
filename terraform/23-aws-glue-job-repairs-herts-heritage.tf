resource "aws_s3_bucket_object" "repairs_herts_heritage_cleaning_script" {
  tags = module.tags.values

  bucket = module.glue_scripts.bucket_id
  key    = "scripts/repairs_herts_heritage_cleaning.py"
  acl    = "private"
  source = "../scripts/repairs_herts_heritage_cleaning.py"
  etag   = filemd5("../scripts/repairs_herts_heritage_cleaning.py")
}

resource "aws_glue_job" "housing_repairs_herts_heritage_cleaning" {
  ...
  command {
    python_version  = "3"
    script_location = "s3://${module.glue_scripts.bucket_id}/${aws_s3_bucket_object.repairs_herts_heritage_cleaning_script.key}"
  }
...
resource "aws_glue_trigger" "housing_repairs_repairs_herts_heritage_cleaning_job" {
  ...
  actions {
    job_name = aws_glue_job.housing_repairs_herts_heritage_cleaning[0].name
  }
}
...
resource "aws_glue_trigger" "housing_repairs_repairs_herts_heritage_cleaning_crawler" {
  ...
  actions {
    crawler_name = aws_glue_crawler.refined_zone_housing_repairs_repairs_herts_heritage_cleaned_crawler.name
  }
}
