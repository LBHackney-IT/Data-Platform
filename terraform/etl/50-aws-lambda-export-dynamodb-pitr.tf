module "export-mtfh-pitr" {
  ...
  environment_variables = {
    SECRET_NAME = aws_secretsmanager_secret.mtfh_export_secret.name
  }
...
locals {
  ...
  "States": {
    "Get Table ARN": {
      ...
      "Next": "Pass",
      "Parameters": {
        "SecretId": "${aws_secretsmanager_secret.mtfh_export_secret.name}"
      },
...
resource "aws_secretsmanager_secret" "mtfh_export_secret" {
  ...
  name = "${local.short_identifier_prefix}mtfh-step-functions"
}
    ...
    "Statement": [
        {
            ...
            "Resource": "${aws_secretsmanager_secret.mtfh_export_secret.arn}"
        }
    ]
}
EOF
...
resource "aws_iam_policy" "role_can_assume_role" {
  name = "${local.short_identifier_prefix}role_can_assume_role"
...
resource "aws_iam_role_policy_attachment" "iam_for_lambda_role_assume_role" {
  role       = module.export-mtfh-pitr[0].lambda_iam_role
  policy_arn = aws_iam_policy.role_can_assume_role.arn
}
