...
resource "aws_glue_job" "housing_repairs_elec_mech_fire_door_entry_cleaning" {
  ...
  command {
    python_version  = "3"
    script_location = "s3://${module.glue_scripts.bucket_id}/${aws_s3_bucket_object.housing_repairs_elec_mech_fire_door_entry_cleaning_script.key}"
  }
...
resource "aws_glue_crawler" "refined_zone_housing_repairs_elec_mech_fire_door_entry_cleaned_crawler" {
  ...
  s3_target {
    path = "s3://${module.refined_zone.bucket_id}/housing-repairs/repairs-electrical-mechanical-fire/door-entry/cleaned/"
...
resource "aws_glue_trigger" "housing_repairs_elec_mech_fire_door_entry_cleaning_job" {
  count = local.is_live_environment ? 1 : 0
  tags  = module.tags.values
  ...
  predicate {
    conditions {
      crawler_name = module.repairs_fire_alarm_aov[0].worksheet_resources["door-entry"].crawler_name
      crawl_state  = "SUCCEEDED"
    }
  }

  actions {
    job_name = aws_glue_job.housing_repairs_elec_mech_fire_door_entry_cleaning[0].name
  }
}
...
resource "aws_glue_trigger" "housing_repairs_elec_mech_fire_door_entry_cleaning_crawler" {
  ...
  actions {
    crawler_name = aws_glue_crawler.refined_zone_housing_repairs_elec_mech_fire_door_entry_cleaned_crawler[0].name
  }
}

resource "aws_glue_trigger" "housing_repairs_elec_mech_fire_door_entry_address_cleaning" {
  count = local.is_live_environment ? 1 : 0

  name          = "${local.identifier_prefix}-housing-repairs-elec-mech-fire-door-entry-address-cleaning-trigger"
  type          = "CONDITIONAL"
  workflow_name = module.repairs_fire_alarm_aov[0].worksheet_resources["door-entry"].workflow_name
  tags          = module.tags.values
  ...
  actions {
    arguments = {
      ...
      "--source_catalog_table" : "housing_repairs_elec_mech_fire_door_entry_cleaned"
      "--cleaned_addresses_s3_bucket_target" : "s3://${module.refined_zone.bucket_id}/housing-repairs/repairs-electrical-mechanical-fire/door-entry/with-cleaned-addresses"
