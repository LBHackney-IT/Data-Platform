...
resource "aws_glue_job" "housing_repairs_repairs_avonline_cleaning" {
  ...
  command {
    python_version  = "3"
    script_location = "s3://${module.glue_scripts.bucket_id}/${aws_s3_bucket_object.housing_repairs_repairs_avonline_cleaning_script.key}"
  }
...
resource "aws_glue_trigger" "housing_repairs_repairs_avonline_cleaning_job" {
  ...
  actions {
    job_name = aws_glue_job.housing_repairs_repairs_avonline_cleaning[0].name
  }
}
...
resource "aws_glue_trigger" "housing_repairs_repairs_avonline_cleaning_crawler" {
  ...
  actions {
    crawler_name = aws_glue_crawler.refined_zone_housing_repairs_repairs_avonline_cleaned_crawler[0].name
  }
}
...
resource "aws_glue_trigger" "housing_repairs_repairs_alpha_track_address_cleaning" {
  ...
  actions {
    arguments = {
      ...
      "--source_catalog_table" : "housing_repairs_repairs_avonline_cleaned"
      "--cleaned_addresses_s3_bucket_target" : "s3://${module.refined_zone.bucket_id}/housing-repairs/repairs-avonline/with-cleaned-addresses"
