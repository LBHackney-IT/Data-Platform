resource "aws_s3_bucket_object" "housing_repairs_elec_mech_fire_lightning_protection_cleaning_script" {
...
resource "aws_glue_job" "housing_repairs_elec_mech_fire_lightning_protection_cleaning" {
  ...
  command {
    python_version  = "3"
    script_location = "s3://${module.glue_scripts.bucket_id}/${aws_s3_bucket_object.housing_repairs_elec_mech_fire_lightning_protection_cleaning_script.key}"
  }

  glue_version = "2.0"

  default_arguments = {
    "--cleaned_repairs_s3_bucket_target" = "s3://${module.refined_zone.bucket_id}/housing-repairs/repairs-electrical-mechanical-fire/lightning-protection/cleaned/"
...
resource "aws_glue_crawler" "refined_zone_housing_repairs_elec_mech_fire_lightning_protection_cleaned_crawler" {
  ...
  s3_target {
    path = "s3://${module.refined_zone.bucket_id}/housing-repairs/repairs-electrical-mechanical-fire/lightning-protection/cleaned/"
...
resource "aws_glue_trigger" "housing_repairs_elec_mech_fire_lightning_protection_cleaning_job" {
  count = local.is_live_environment ? 1 : 0
  tags  = module.tags.values
  ...
  actions {
    job_name = aws_glue_job.housing_repairs_elec_mech_fire_lightning_protection_cleaning[0].name
  }
}
...
resource "aws_glue_trigger" "housing_repairs_elec_mech_fire_lightning_protection_cleaning_crawler" {
  ...
  actions {
    crawler_name = aws_glue_crawler.refined_zone_housing_repairs_elec_mech_fire_lightning_protection_cleaned_crawler[0].name
  }
}

resource "aws_glue_trigger" "housing_repairs_elec_mech_fire_lightning_protection_address_cleaning" {
  count = local.is_live_environment ? 1 : 0

  name          = "${local.identifier_prefix}-housing-repairs-elec-mech-fire-lightning-protection-address-cleaning-trigger"
  type          = "CONDITIONAL"
  workflow_name = module.repairs_fire_alarm_aov[0].worksheet_resources["lightning-protection"].workflow_name
  tags          = module.tags.values
  ...
  actions {
    arguments = {
      ...
      "--source_catalog_table" : "housing_repairs_elec_mech_fire_lightning_protection_cleaned"
      "--cleaned_addresses_s3_bucket_target" : "s3://${module.refined_zone.bucket_id}/housing-repairs/repairs-electrical-mechanical-fire/lightning-protection/with-cleaned-addresses"
