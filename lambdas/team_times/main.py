...
def save_xml_to_s3(xml_data, bucket_name, object_name):
    s3_client.put_object(Body=xml_data, Bucket=bucket_name, Key=object_name)
    print(f"XML saved to s3://{bucket_name}/{object_name}")


def save_csv_to_s3(csv_data, bucket_name, object_name):
    s3_client.put_object(Body=csv_data.getvalue(), Bucket=bucket_name, Key=object_name)
...
def make_api_request_and_process_data(api_endpoint, api_key, date, data_frequency):
    ...
    if response.status_code == 200:
        ...
        save_xml_to_s3(response.text, xml_bucket_name, xml_object_name)

        csv_data = xml_to_csv(response.text)
        save_csv_to_s3(csv_data, csv_bucket_name, csv_object_name)
    else:
