...
async function checkS3ForFile() {
  ...
  const params = {
    Bucket: s3Bucket,
    Key: `${objectKeyPrefix}ringo_dump_${YYMMDD()}.zip`,
  };
  ...
  try {
    ...
    console.log("Today's Ringo file not yet present in S3 bucket, retrieving file from SFTP")
    return false;
  }
}
...
exports.handler = async () => {
  ...
  if (await checkS3ForFile()) {
    console.log("Today's ringo file is already present in S3 bucket!");
