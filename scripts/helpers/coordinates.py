...
def OSGB36toWGS84(E,N):
    """
    Transformation described by OrdnanceSurvey at https://www.ordnancesurvey.co.uk/business-government/tools-support/os-net/for-developers
    Code copied from Convertbng project https://github.com/urschrei/lonlat_bng/blob/master/src/bng_lonlat.py
    """
...
def convert_bng_to_latlon(df,easting_column_name, northing_column_name):  
    df = df.withColumn(easting_column_name,df[easting_column_name].cast("double"))
    df = df.withColumn(northing_column_name,df[northing_column_name].cast("double"))
    df = df.withColumn('latlon', udfConvert(df[easting_column_name], df[northing_column_name]))
