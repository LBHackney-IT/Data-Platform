### ETL script:                 etl_zerobase_ctax_live_properties.py
...
### Based upon:                 AWSGlue-Revenues-ETL-CTax_Live_Properties_Initialization.SparkSQL.py
### Taken from AWS Glue job:    Revenues ETL CTax_Live_Properties_Initialization
...
# Script generated for node Jx_ID_1_Initialization.Spark.sql
SqlQuery2 = """
-- Last updated:        2022-05-31
-- Spark SQL query:     Jx_ID_1_Initialization.Spark.sql
---
-- AWS Glue job:        Revenues ETL CTax_Live_Properties_Initialization
---
-- Jx database:         Defined externally
-- Jx source:           Established programatically
...
---
-- AWS Glue Studio forces us to define an import source, tho an import is not strictly needed for this initialisation.
-- However selecting a data source at the top of the ETL will ensure this simple transform produces an appropriate current_timestamp() value which is close as possible to the strat of the ETL process.
-- Therefore after the script is generating check to see which import source is introduced first and use that as the import source.
SELECT
    CAST(1 AS INT)                                      AS Jx_ID,           -- Job's index ID = Number ONE
    CAST(0 AS INT)                                      AS generation_ID,   -- Generation ZERO at initialization (exclusively)
    'Revenues ETL CTax_Live_Properties_Initialization'  AS transform_name,  -- In theory this could be anything so long as both ID columns are used to reference the transform
    ...
    'Jx_ID_1_Initialization.Spark.sql: '                -- Reproducing the critical outputs to monitor...
        || 'CAST(1 AS INT) --> Jx_ID = ' || STRING( CAST(1 AS INT) ) || '; '
        || 'CAST(0 AS INT) --> generation_ID = ' || STRING( CAST(0 AS INT) ) || '; '
        AS debug_info
--;--

"""
Jx_ID_1_InitializationSparksql_node1652882136682 = sparkSqlQuery(
    glueContext,
    query=SqlQuery2,
    mapping={"dummy": lbhaliverbviews_core_ctaccount_node1651057095386},
    transformation_ctx="Jx_ID_1_InitializationSparksql_node1652882136682",
)
...
# Script generated for node CTax_Live_Properties_Initialization.Spark.sql
SqlQuery1 = """
-- Last updated:        2022-05-23
-- Spark SQL query:     CTax_Live_Properties_Initialization.Spark.sql
---
-- AWS Glue job:        Revenues ETL CTax_Live_Properties_Initialization
-- Job bookmark:        Disable
---
-- Source database:     revenues-raw-zone
...
--;--
"""
CTax_Live_Properties_InitializationSparksql_node1650654183012 = sparkSqlQuery(
    glueContext,
    query=SqlQuery1,
    mapping={
        ...
        "dummy": Jx_ID_1_InitializationSparksql_node1652882136682,
    },
    transformation_ctx="CTax_Live_Properties_InitializationSparksql_node1650654183012",
)
...
Jx_DataSparksql_node1653307221832 = sparkSqlQuery(
    ...
    query=SqlQuery3,
    mapping={
        "data": CTax_Live_Properties_InitializationSparksql_node1650654183012,
        "jobs": Jx_ID_1_InitializationSparksql_node1652882136682,
    },
