...
dq_params = {'appeals': {'unique': ['id'], 'complete': 'id'},
             'applications': {'unique': ['id'], 'complete': 'application_reference_number'},
             'appeal_decision': {'unique': ['id'], 'complete': 'id'},
             'appeal_status': {'unique': ['id'], 'complete': 'id'},
             'appeal_types': {'unique': ['id'], 'complete': 'id'},
             'committees': {'unique': ['id'], 'complete': 'id'},
             'communications': {'unique': ['id'], 'complete': 'id'},
             'communication_types': {'unique': ['id'], 'complete': 'id'},
             'contacts': {'unique': ['id'], 'complete': 'id'},
             'contact_types': {'unique': ['id'], 'complete': 'id'},
             'decision_types': {'unique': ['id'], 'complete': 'id'},
             'documents': {'unique': ['id'], 'complete': 'id'},
             'document_types': {'unique': ['id'], 'complete': 'id'},
             'dtf_locations': {'unique': ['id'], 'complete': 'id'},
             'emails': {'unique': ['id'], 'complete': 'id'},
             'enforcements': {'unique': ['id'], 'complete': 'id'},
             'fees': {'unique': ['id'], 'complete': 'id'},
             'fee_payments': {'unique': ['id'], 'complete': 'id'},
             'fee_types': {'unique': ['id'], 'complete': 'id'},
             'ps_development_codes': {'unique': ['id'], 'complete': 'id'},
             'public_comments': {'unique': ['id'], 'complete': 'id'},
             'public_consultations': {'unique': ['id'], 'complete': 'document_id'},
             'users': {'unique': ['id'], 'complete': 'id'},
             'committee_application_map': {'unique': ['id'], 'complete': 'id'},
             'user_teams': {'unique': ['id'], 'complete': 'id'},
             'user_team_map': {'unique': ['id'], 'complete': 'id'},
             'application_types': {'unique': ['id'], 'complete': 'id'},
             'pre_applications': {'unique': ['id'], 'complete': 'id'},
             'pre_application_categories': {'unique': ['id'], 'complete': 'id'},
        ...
        for table_name in table_list:
            ...
            if not table_exists_in_catalog(glueContext, snapshot_table_name, source_catalog_database):
                ...
                if not table_exists_in_catalog(glueContext, increment_table_name, source_catalog_database):
                    ...
                    continue
                increment_df = loadIncrementsSinceDate(increment_table_name=increment_table_name,
                                                       name_space=source_catalog_database, date="20210101")
            ...
            # snapshot table in glue catalogue
                ...
                if table_exists_in_catalog(glueContext, increment_table_name, source_catalog_database):
                    increment_df = loadIncrementsSinceDate(increment_table_name=increment_table_name,
                                                           name_space=source_catalog_database, date=last_snapshot_date)
                    if increment_df.rdd.isEmpty():
            ...
            if (dq_params.get(snapshot_table_name, {}).get("unique")):
                check = check.hasUniqueness(dq_params[snapshot_table_name]["unique"], lambda x: x == 1,
                                            f'{dq_params[snapshot_table_name]["unique"]} are not unique')
            if (dq_params.get(snapshot_table_name, {}).get("complete")):
                check = check.hasCompleteness(dq_params[snapshot_table_name]["complete"], lambda x: x >= 0.99,
            ...
            try:
                ...
                dq_errors.append(f'Job for table {snapshot_table_name} cancelled due to data quality test failure.')
