...
def createPushDownPredicate(daysBuffer):
    if daysBuffer > 0:
        pushDownPredicate = f"snapshot_date>=date_format(date_sub(current_date, {daysBuffer}), 'yyyyMMdd')"
    else:
        pushDownPredicate = ''
...
if __name__ == "__main__":
    ...
    for nameOfTableToRecast in table_list:
        ...
        if not table_exists_in_catalog(glueContext, snapshot_table_name, source_catalog_database):
            logger.info(f"Couldn't find table {snapshot_table_name} in database {source_catalog_database}, moving onto next table.")
            continue
        ...
        # load increments later than the last snapshot date
        ...
        NEW_PARTITION_KEYS = ["snapshot_year", "snapshot_month", "snapshot_day", "snapshot_date"]
        ...
        parquetData = glueContext.write_dynamic_frame.from_options(
            ...
            connection_type="s3",
            format="parquet",
            connection_options={"path": target_destination, "partitionKeys": NEW_PARTITION_KEYS},
