...
from scripts.helpers.helpers import get_glue_env_var, get_latest_partitions, create_pushdown_predicate, add_import_time_columns, table_exists_in_catalog, clear_target_folder, PARTITION_KEYS
...
'''Function not used at the moment. It assumes the input dataframe has columns named (lat, lon) or (latitude, longitude) or (easting, northing) or (eastings, northings). Turns the dataframe containing points into a geopandas dataframe and generate other coords columns so we always have latitude/longitude and eastings/northings'''
...
def create_geom_and_extra_coords(pandas_df, source_crs, target_crs, x_column, y_column, logger):
    ...
    geopandas_df = geopandas.GeoDataFrame(point_df, crs=source_crs, geometry=geopandas.points_from_xy(pandas_df[x_column], pandas_df[y_column]))
...
'''Replaces nan values with '' in a pandas dataframe, so it can be converted to spark datafrane later even for non 'string' columns'''
...
if __name__ == "__main__":
    ...
    args = getResolvedOptions(sys.argv, ['TempDir','JOB_NAME','tables_to_enrich_dict_path','geography_tables_dict_path'])
    ...
    for geography_table in geography_tables_dict:
        ...
        for column_to_append in geography_table["columns_to_append"]:
            geography_df = geography_df.withColumnRenamed(column_to_append["column_name"], column_to_append["column_alias"])
    ...
    for enrich_table in enrich_tables_dict:
        ...
        if "geom_column" in enrich_table:
            ...
            if geom_format == "wkt":
                enrich_geo_df = geopandas.GeoDataFrame(point_df, crs=source_crs, geometry=point_df[geom_column_name].apply(wkt_loads))
            elif geom_format == "wkb":
                enrich_geo_df = geopandas.GeoDataFrame(point_df, crs=source_crs, geometry=point_df[geom_column_name].apply(shapely.wkb.loads, args=(True,)))
            else:
        ...
        for geography_table_key in geography_df_dict:
            if geography_table_key in enrich_table["enrich_with"]:
                ...
                enrich_geo_df = geopandas.sjoin(enrich_geo_df, geography_df, how='left', predicate='intersects', lsuffix='left', rsuffix='right')
        ...
        # deal with nan
        enrich_pd_df = deal_with_nan_colums(enrich_pd_df,geography_tables_dict)
