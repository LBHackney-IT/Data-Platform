...
if __name__ == "__main__":
    ...
    exist = s3.list_objects_v2(
        Bucket=s3_landing, Prefix="housing_export/rentsense/"
    )  # list the files
    ...
    exist2 = s3.list_objects_v2(
        Bucket=s3_bucket, Prefix="housing/rentsense/export/"
    )  # list the files
    ...
    exist3 = s3.list_objects_v2(
        Bucket=s3_bucket, Prefix="housing/rentsense/gzip/"
    )  # list the files
    ...
    patch_officer = patch_officer.withColumn(
        "patch_number2", F.trim(F.col("patch_number"))
    )

    patch = (
        dfprop.withColumn("patch2", F.trim(F.col("arr_patch")))
        .withColumn("property_ref", F.trim(F.col("prop_ref")))
        .replace(to_replace=mapPatch, subset=["patch2"])
    )

    patch = patch.join(
        patch_officer, patch.patch2 == patch_officer.patch_number2, "left"
    )
    patch = patch.selectExpr(
        "property_ref as prop_ref",
        "patch2 as Patch",
    ...
    accounts = df2.filter(
        "((endoftenuredate IS NULL) OR (endoftenuredate > current_date())) AND paymentreference <> ''"
    )
    ...
    accounts2 = accounts2.join(
        df_agg, accounts2.AccountReference == df_agg.AccountR, "left"
    )

    # add the breathing space details
    case_priorities = case_priorities.filter(F.col("is_paused_until") > today)
    case_priorities = (
        case_priorities.withColumn("BreathingSpace", lit(1))
        .drop("import_date")
        .withColumnRenamed("tenancy_ref", "tenancy_ref2")
    )

    accounts2 = accounts2.join(
        case_priorities, accounts2.tenancy_ref == case_priorities.tenancy_ref2, "left"
    )
    accounts2 = accounts2.selectExpr(
        "AccountReference as PaymentReference",
        "TenureType",
        "TenureTypeCode",
    ...
    dynamic_frame = DynamicFrame.fromDF(
        accounts2.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/accounts",
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        connection_options={
            "path": s3_bucket_target + "/gzip/accounts",
            "compression": "gzip",
    ...
    copy_file(
        s3_bucket, export_target_source, filename, s3_landing, target_path, filename
    )
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/arrangements",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format_options={"separator": ","},
        connection_options={
            "path": s3_bucket_target + "/gzip/arrangements",
            "compression": "gzip",
            "partitionKeys": ["import_date"],
        },
        transformation_ctx="target_data_to_write",
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format_options={"separator": ","},
        connection_options={
            "path": s3_bucket_target + "/gzip/tenants",
            "compression": "gzip",
            "partitionKeys": ["import_date"],
        },
        transformation_ctx="target_data_to_write",
    ...
    bals = balance.withColumn(
        "BalanceDate", to_date(col("import_date"), "yyyyMMdd")
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/balances",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format_options={"separator": ","},
        connection_options={
            "path": s3_bucket_target + "/gzip/balances",
            "compression": "gzip",
            "partitionKeys": ["import_date"],
        },
        transformation_ctx="target_data_to_write",
    ...
    actions = df9.withColumn("uh_ten_ref1", F.trim(col("tag_ref"))).withColumn(
        "ActionDate", F.to_date(col("action_date"), "yyyy-MM-dd")
    )
    actions = actions.filter(
        (F.col("action_date") > date_sub(current_date(), 180))
        & (col("action_date") < current_date())
    )
    actions = ten.join(actions, ten.uh_ten_ref == actions.uh_ten_ref1, "inner")
    actions = actions.withColumn("code_lookup", trim(col("action_code"))).replace(
        to_replace=mapAction, subset=["code_lookup"]
    )

    actions = actions.selectExpr(
        "paymentreference as PaymentReference",
        "tag_ref",
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format_options={"separator": ","},
        connection_options={
            "path": s3_bucket_target + "/gzip/actions",
            "compression": "gzip",
            "partitionKeys": ["import_date"],
        },
        transformation_ctx="target_data_to_write",
    ...
    df11 = df10.filter(
        (F.col("post_date") > F.date_sub(F.current_date(), 180))
        & (col("post_date") < F.current_date())
    )
    df11 = (
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/transactions",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    glueContext.write_dynamic_frame.from_options(
        ...
        format_options={"separator": ","},
        connection_options={
            "path": s3_bucket_target + "/gzip/transactions",
            "compression": "gzip",
            "partitionKeys": ["import_date"],
        },
        transformation_ctx="target_data_to_write",
