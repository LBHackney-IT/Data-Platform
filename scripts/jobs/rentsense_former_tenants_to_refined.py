...
def export_dynamic_frame_as_xml_gzip(
    dynamic_frame: DynamicFrame, s3_bucket: str, target_path: str, file_name: str
) -> None:
    """
    ...
    dynamic_frame = DropFields.apply(
        dynamic_frame,
        paths=[
            "import_datetime",
            "import_timestamp",
    ...
    s3_client.upload_fileobj(
        xml_buffer, s3_bucket, f"{target_path}/rent.{file_name}{today}.xml.gz"
    )
...
if __name__ == "__main__":
    ...
    exist = s3.list_objects_v2(
        Bucket=s3_landing, Prefix="housing_export/rentsense-ft/"
    )  # list the files
    ...
    exist2 = s3.list_objects_v2(
        Bucket=s3_bucket, Prefix="housing/rentsense-ft/export/"
    )  # list the files
    ...
    exist3 = s3.list_objects_v2(
        Bucket=s3_bucket, Prefix="housing/rentsense-ft/gzip/"
    )  # list the files
    ...
    accounts = df2.filter(
        "endoftenuredate is not NULL and paymentreference<>'' and paymentreference<>'Blank - not in use'"
    )
    accounts_s = accounts.where(
        col("description").isin(
            {"Secure", "Non-Secure", "Private Garage", "Tenant Garage"}
        )
    )
    ...
    accounts_int = accounts_int.join(
        accounts_s,
        accounts_int.paymentreference == accounts_s.paymentreference,
        "leftanti",
    )  # remove the paymentreference in the other dataset

    accounts = accounts_s.union(accounts_int)
    accounts.select(
        col("startOfTenureDate"),
        to_date(col("startOfTenureDate"), "yyyy-MM-dd").alias("date"),
    ...
    start_ten = accounts.selectExpr(
        "paymentreference as AccountR", "left(startOfTenureDate,10) as TenancyDate"
    )

    df_agg = start_ten.groupBy("AccountR").agg(F.max("TenancyDate").alias("max_date"))

    # payref to tenancy ref

    p_ref = df7.selectExpr(
        "trim(u_saff_rentacc) as pay_ref", "trim(tag_ref) as uh_ten_ref"
    )

    accounts = accounts.join(p_ref, accounts.paymentreference == p_ref.pay_ref, "left")

    bals_filter = (
        balance.withColumn("BalanceDate", F.to_date(F.col("import_date"), "yyyyMMdd"))
    ...
    accounts2 = accounts.join(
        bals_filter, accounts.paymentreference == bals_filter.paymentreference2, "right"
    )

    accounts2 = accounts2.filter("paymentreference is not null").withColumn(
        "TenancyEndDate", F.to_date(substring("endoftenuredate", 1, 10), "yyyy-MM-dd")
    )
    ...
    ten = accounts2.select("uh_ten_ref", "paymentreference").filter(
        col("previousweekbalance") > 0.00
    )
    ...
    agg_end = agg_end.selectExpr("agreement_id", "created_at as AgreementEndDate")

    arr = arr.join(agg_end, arr.id == agg_end.agreement_id, "left")
    arr = (
        arr.withColumn(
            "AgreementStartDate", F.to_date(F.col("start_date"), "yyyy-MM-dd")
        )
        .withColumn(
            "AgreementEndDate1", F.to_date(F.col("AgreementEndDate"), "yyyy-MM-dd")
        )
        .withColumn(
            "AgreementCreatedDate", F.to_date(F.col("created_at"), "yyyy-MM-dd")
        )
        .withColumn(
            "AgreementCode", F.when(F.col("court_case_id") > 0, "C").otherwise("N")
        )
    ...
    dynamic_frame = DynamicFrame.fromDF(
        arr.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    parquet_data = glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/formerarrangements",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )

    export_dynamic_frame_as_xml_gzip(
        dynamic_frame, s3_bucket, target_path, "formerarrangements"
    )

    # Tenants
    tens = accounts2.filter("member_is_responsible like true")

    asset = df3.selectExpr(
        "asset_id",
    ...
    mob = df4.filter(
        (df4.contacttype == "phone")
        & (df4.subtype == "mobile")
        & (df4.isactive == "true")
    ).selectExpr("person_id as pid1", "value as MobileNumber")
    # get one record
    mob = mob.groupBy("pid1").agg(F.max("MobileNumber").alias("MobileNumber"))

    lline = df4.filter(
        (df4.contacttype == "phone")
        & (df4.subtype == "landline")
        & (df4.isactive == "true")
    ).selectExpr("person_id  as pid2", "value as TelephoneNumber")

    lline = lline.groupBy("pid2").agg(F.max("TelephoneNumber").alias("TelephoneNumber"))

    email = df4.filter(df4.contacttype == "email").selectExpr(
        "person_id  as pid3", "value as Email"
    )

    email = email.groupBy("pid3").agg(F.max("Email").alias("Email"))

    # add the correspondence address
    cor = df4.filter(
        (df4.subtype == "correspondenceAddress") & (df4.isactive == "true")
    ).selectExpr("person_id as pid4", "value as correspondenceAddress")
    ...
    dynamic_frame = DynamicFrame.fromDF(
        tens.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    parquet_data = glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/formertenants",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    export_dynamic_frame_as_xml_gzip(
        dynamic_frame, s3_bucket, target_path, "formertenants"
    )

    # Balances
    ten = accounts2.select("uh_ten_ref", "paymentreference")

    bals = balance.withColumn(
        "BalanceDate", F.to_date(F.col("import_date"), "yyyyMMdd")
    ).withColumn("paymentreference2", F.trim(F.col("RentAccount")))
    ...
    dynamic_frame = DynamicFrame.fromDF(
        balances.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    parquet_data = glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/formerbalances",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    export_dynamic_frame_as_xml_gzip(
        dynamic_frame, s3_bucket, target_path, "formerbalances"
    )
    ...
    actions = (
        df9.withColumn("uh_ten_ref1", F.trim(F.col("tag_ref")))
        .withColumn("timestamp", to_timestamp("action_date", "yyyy-MM-dd HH:mm:ss.S"))
        .withColumn(
            "ActionDate", date_format("timestamp", "yyyy-MM-dd'T'HH:mm:ss.SSSZ")
        )
    )
    ...
    actions = actions.filter(
        (F.col("action_date") > date_sub(current_date(), 365))
        & (F.col("action_date") < current_date())
    )

    actions = ten.join(actions, ten.uh_ten_ref == actions.uh_ten_ref1, "inner")

    actions = actions.withColumn("code_lookup", F.trim(F.col("action_code"))).replace(
        to_replace=mapAction, subset=["code_lookup"]
    )

    max_action = (
        actions.groupBy("paymentreference").agg(F.max("action_no").alias("max_action"))
    ).withColumnRenamed("paymentreference", "payref")

    actions1 = actions.join(
        max_action, actions.paymentreference == max_action.payref, "inner"
    )

    latest = actions1.filter("max_action=action_no")

    actions = latest.selectExpr(
        "paymentreference as AccountReference",
    ...
    export_dynamic_frame_as_xml_gzip(
        dynamic_frame, s3_bucket, target_path, "formeractions"
    )

    actions = add_import_time_columns(actions)

    dynamic_frame = DynamicFrame.fromDF(
        actions.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    parquet_data = glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/formeractions",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    accounts2 = accounts2.withColumn(
        "days_since_termination", datediff(current_date(), F.col("TenancyEndDate"))
    )

    accounts3 = accounts2.withColumn(
        "Patch",
        when(accounts2["days_since_termination"] < 7, "New Terminations")
        .when(
            (accounts2["days_since_termination"] >= 7)
            & (accounts2["days_since_termination"] <= 31),
            "0-1 month",
        )
        .when(
            (accounts2["days_since_termination"] > 31)
            & (accounts2["days_since_termination"] <= 92),
            "2-3 months",
        )
        .when(
            (accounts2["days_since_termination"] > 92)
            & (accounts2["days_since_termination"] <= 182),
            "3-6 months",
        )
        .when(
            (accounts2["days_since_termination"] > 182)
            & (accounts2["days_since_termination"] <= 365),
            "6-12 months",
        )
        .when(
            (accounts2["days_since_termination"] > 365)
            & (accounts2["days_since_termination"] <= 2191),
            "1 year - 6 years",
        )
    ...
    accounts3 = accounts3.selectExpr(
        "paymentreference as AccountReference",
    ...
    accounts4 = accounts3.join(
        df_agg, accounts3.AccountReference == df_agg.AccountR, "left"
    )

    case_priorities = case_priorities.filter(F.col("pause_reason") == "Deceased")

    case_priorities = (
        case_priorities.withColumn("Deceased", lit(1))
        .drop("import_date")
        .withColumnRenamed("tenancy_ref", "tenancy_ref2")
    )

    accounts5 = accounts4.join(
    ...
    dynamic_frame = DynamicFrame.fromDF(
        accounts9.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    parquet_data = glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/formeraccounts",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    # Export to .xml.gzip
    export_dynamic_frame_as_xml_gzip(
        dynamic_frame, s3_bucket, target_path, "formeraccounts"
    )
    # Transactions
    ...
    df11 = df10.filter(
        (F.col("post_date") > date_sub(current_date(), 365))
        & (F.col("post_date") < current_date())
    )

    df11 = (
        df11.withColumn("TransactionID", F.monotonically_increasing_id())
    ...
    trans1 = trans.withColumn(
        "timestamp", to_timestamp("post_date", "yyyy-MM-dd HH:mm:ss.S")
    ).withColumn(
        "iso8601_format", date_format("timestamp", "yyyy-MM-dd'T'HH:mm:ss.SSSZ")
    )

    trans = trans1.selectExpr(
        "paymentreference as AccountReference",
        "TransactionID",
    ...
    dynamic_frame = DynamicFrame.fromDF(
        trans.repartition(1), glueContext, "target_data_to_write"
    )
    ...
    parquet_data = glueContext.write_dynamic_frame.from_options(
        ...
        format="parquet",
        connection_options={
            "path": s3_bucket_target + "/formertransactions",
            "partitionKeys": PARTITION_KEYS,
        },
        transformation_ctx="target_data_to_write",
    )
    ...
    export_dynamic_frame_as_xml_gzip(
        dynamic_frame, s3_bucket, target_path, "formertransactions"
    )
