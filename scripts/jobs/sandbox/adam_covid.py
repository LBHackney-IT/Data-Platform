...
if __name__ == "__main__":
    ...
    df = renameLocationCol(df)
    df = convertStringToDate(df)
    df2 = renameLocationCol(df2)
    
    # drop unnecessary columns so they are not duplicated in the join
    
    df2 = df2.drop("import_datetime","import_timestamp","import_year","import_month","import_day","import_date")
    
    # join the dataset
    
    df = df.join(df2,df.country ==  df2.country,"inner")
