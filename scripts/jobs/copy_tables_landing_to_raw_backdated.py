...
from scripts.helpers.helpers import get_glue_env_var, add_timestamp_column_from_date, PARTITION_KEYS
...
import_date = workflow_params['import_date']
...
tables_to_copy = [table for table in glue_context.tableNames(dbName=database_name_source) if
                  filtering_pattern.match(table)]
logger.info(f"Tables to copy: {tables_to_copy}")
logger.info(f"Number of tables to copy: {len(tables_to_copy)}")
logger.info(f"Partition date to copy:{import_date}")
...
for table_name in tables_to_copy:
    ...
    table_data_frame = glue_context.create_dynamic_frame.from_catalog(
        ...
        push_down_predicate=f"import_date={import_date}"
    ).toDF()
    ...
    import_date_as_datetime = datetime.strptime(import_date, '%Y%m%d')
    table_with_timestamp = add_timestamp_column_from_date(table_data_frame, import_date_as_datetime)

    # resultDataFrame = DynamicFrame.fromDF(table_with_timestamp, glue_context, "result_dataframe")
    # target_destination = "s3://" + bucket_target + "/" + prefix + table_name
