...
if __name__ == "__main__":
    ...
    patch = patch.withColumn("patch2", F.trim(F.col("patch"))).withColumn(
        "payref", F.trim(F.col("ref"))
    )
    patch = patch.join(
        patch_officer, patch.patch2 == patch_officer.patch_number2, "left"
    )
    patch = patch.selectExpr(
        "payref", "patch2 as Patch", "officer_full_name as HousingOfficerName"
    )
    ...
    actions = df9.withColumn("uh_ten_ref1", F.trim(F.col("tag_ref"))).withColumn(
        "ActionDate", F.to_date(F.col("action_date"), "yyyy-MM-dd")
    )
    ...
    df11 = df10.filter(
        (F.col("post_date") > date_sub(current_date(), 180))
