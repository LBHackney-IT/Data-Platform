...
from scripts.jobs.env_context import (
    DEFAULT_MODE_AWS,
    LOCAL_MODE,
    ExecutionContextProvider,
)
from scripts.jobs.data_and_insight.person_matching_module import (
    remove_deceased,
    prepare_clean_council_tax_data,
    standardize_council_tax_data,
    prepare_clean_housing_benefit_data,
    standardize_housing_benefit_data,
    prepare_clean_parking_permit_data,
    standardize_parking_permit_data,
    prepare_clean_housing_data,
...
def main():
    ...
    parser.add_argument(
        "--execution_mode",
        default=DEFAULT_MODE_AWS,
        choices=[DEFAULT_MODE_AWS, LOCAL_MODE],
        type=str,
        required=False,
        metavar="set --execution_mode=aws to run on AWS",
    )
    ...
    source_catalog_table_ctax_liability_person_glue_arg = (
        "source_catalog_table_ctax_liability_person"
    )
    source_catalog_table_ctax_non_liability_person_glue_arg = (
        "source_catalog_table_ctax_non_liability_person"
    )
    ...
    glue_args = [
        source_catalog_database_housing_glue_arg,
    ...
    with ExecutionContextProvider(mode, glue_args, local_args) as execution_context:
        ...
        source_catalog_database_housing = execution_context.get_input_args(
            source_catalog_database_housing_glue_arg
        )
        ...
        logger.info("housing df cleaned and standardised")

        # # get Council Tax data
        source_catalog_database_council_tax = execution_context.get_input_args(
            source_catalog_database_council_tax_glue_arg
        )
        source_catalog_table_ctax_account = execution_context.get_input_args(
            source_catalog_table_ctax_account_glue_arg
        )
        ...
        source_catalog_table_ctax_non_liability_person = (
            execution_context.get_input_args(
                source_catalog_table_ctax_non_liability_person_glue_arg
            )
        )
        ...
        council_tax_cleaned = prepare_clean_council_tax_data(
            spark,
            council_tax_account_df,
            council_tax_liability_person_df,
            council_tax_non_liability_person_df,
            council_tax_occupation_df,
            council_tax_property_df,
        )
        council_tax_cleaned = remove_deceased(council_tax_cleaned)
        council_tax = standardize_council_tax_data(council_tax_cleaned)

        logger.info("council_tax df cleaned and standardised")

        # get housing benefit data
        source_catalog_database_benefits = execution_context.get_input_args(
            source_catalog_database_benefits_glue_arg
        )
        source_catalog_table_hb_member = execution_context.get_input_args(
            source_catalog_table_hb_member_glue_arg
        )
        source_catalog_table_hb_household = execution_context.get_input_args(
            source_catalog_table_hb_household_glue_arg
        )
        ...
        hb_member_df = execution_context.get_dataframe(
            name_space=source_catalog_database_benefits,
        ...
        hb_ctax_assessment_df = execution_context.get_dataframe(
            name_space=source_catalog_database_benefits,
            table_name=source_catalog_table_hb_ctax_assessment,
        )

        housing_benefit_cleaned = prepare_clean_housing_benefit_data(
            hb_member_df, hb_household_df, hb_rent_assessment_df, hb_ctax_assessment_df
        )
        ...
        logger.info("Housing benefit df cleaned and standardised")

        # get parking data
        source_catalog_database_parking = execution_context.get_input_args(
            source_catalog_database_parking_glue_arg
        )
        ...
        logger.info("Parking df cleaned and standardised")
        logger.info(f"Parking preview: {parking_permit.show(10, truncate=False)}")

        logger.info("Starting to union dataframes...")
        standard_df = (
            housing.union(council_tax)
            .union(housing_benefit)
            .union(parking_permit)
            .coalesce(10)
        )
        logger.info("Standard df ready...starting dq tests")
        ...
        id_uniqueness_result = df_ge.expect_column_values_to_be_unique(
            column="source_id", result_format={"result_format": "COMPLETE"}
        )
        ...
        standard_df = standard_df.withColumn(
            "is_duplicated",
            when(col("source_id").isin(duplicate_ids), True).otherwise(False),
        )
        # check for completeness
        id_not_null_result = df_ge.expect_column_values_to_not_be_null(
            column="source_id", result_format={"result_format": "COMPLETE"}
        )
        ...
        execution_context.save_dataframe(
            standard_df, output_path, *PARTITION_KEYS, save_mode="overwrite"
        )
