...
AmazonS3parkingraweta_decision_records_node1645813232451 = (
    glueContext.create_dynamic_frame.from_catalog(
        database="parking-raw-zone",
        table_name="eta_decision_records",
        transformation_ctx="AmazonS3parkingraweta_decision_records_node1645813232451",
    )
)
...
,sum(Case when decision_i_e_dnc_appeal_allowed_appeal_rejected in ('Appeal with Direction') then 1 else 0 end) as appeal_with_direction
...
,Sum(Case 
     ...
     and isvoid = 0 
     and warningflag  = 0 
     and eta_recs.appeal_rejected > 1
...
,Sum(Case 
     ...
     and isvoid = 0 
     and warningflag  = 0 
     and eta_recs.appeal_with_direction > 1
...
,Sum(Case 
     ...
     and isvoid = 0 
     and warningflag  = 0 
     and (eta_recs.appeal_rejected > 1 or eta_recs.appeal_allowed > 1 or eta_recs.appeal_dnc > 1 or eta_recs.appeal_with_direction > 1)
...
ApplyMapping_node2 = sparkSqlQuery(
    ...
    mapping={
        ...
        "eta_decision_records": AmazonS3parkingraweta_decision_records_node1645813232451,
    },
...
S3bucket_node3 = glueContext.getSink(
    path="s3://dataplatform-" + environment + "-refined-zone/parking/liberator/parking_kpi_gds_summary/",
    connection_type="s3",
