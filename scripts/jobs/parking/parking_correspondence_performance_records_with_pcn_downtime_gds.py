...
table_name = "parking_correspondence_performance_records_with_pcn_downtime_gds"

# The exact same query prototyped in pre-prod(stg) or prod Athena
query_on_athena = """
-->> "dataplatform-prod-liberator-refined-zone"."parking_correspondence_performance_records_with_pcn_downtime_gds" 
...
05/11/2024 + Refactored SQL to separate the inward column conversions from onward business logic using CTE subqueries and allow improved fault handling.
...
05/03/2025 - Removed redundant left joins from subquery
...
28/04/2055 - Checked over logic of conditionally-shown columns
*/
...
responses AS (
    SELECT
        ...
        l.Response_written_by AS tot_rep_Response_written_by,
        SUBSTR(l.response_generated_at, 1, 10) AS tot_rep_response_date,   
        SUBSTR(l.response_generated_at, 1, 7) || '-01' AS tot_rep_monthYear_response,
        -- Empty "response_generated_at" outputs '-01' which would get counted!
        -- Makes no difference: SUBSTR(CAST(liberator_pcn_ic.response_generated_at AS VARCHAR(10)), 1, 7) || '-01' AS tot_rep_monthYear_response,
        l.Response_written_by || SUBSTR(l.response_generated_at, 1, 10) AS tot_rep_unique_link,

        /* total_response_month*/
        l.Response_written_by AS mth_tot_rep_Response_written_by,
        SUBSTR(l.response_generated_at, 1, 10) AS mth_tot_rep_response_date,   
        SUBSTR(l.response_generated_at, 1, 7) || '-01' AS mth_tot_rep_monthYear_response,
        l.Response_written_by || SUBSTR(l.response_generated_at, 1, 7) || '-01' AS mth_tot_rep_unique_link

        -- Q. How do empty "response_generated_at" rows effect subsequent business logic?  
    FROM "dataplatform-prod-liberator-raw-zone"."liberator_pcn_ic" l
        -- The unreferenced p. caused SQL linter error ST11 
        -- Then discoverd the row count was not expanded by the left join to p because p.pcn is unique within import_date
...
*/
    WHERE l.import_Date IN (
            SELECT MAX(g.import_date) AS import_date
            FROM "dataplatform-prod-liberator-raw-zone"."liberator_pcn_ic" g
        )
    AND l.response_generated_at <> '' -- Q. Should I remove empty "response_generated_at" to prevent side-effects?
    AND LENGTH(l.ticketserialnumber) = 10
    AND CAST(SUBSTR(l.date_received, 1, 10) AS DATE) > CURRENT_DATE - INTERVAL '13' MONTH 
        -- Last 13 months from todays date
/*      -- This alternative method captures slightly more records...
        DATE_DIFF(
            'month',
            TRY(CAST(SUBSTR(l.date_received, 1, 10) AS DATE)),
...
officer_total_rep_dates AS (
    SELECT
        tot_rep_Response_written_by,
        tot_rep_response_date,
        tot_rep_monthYear_response,
        tot_rep_unique_link,

        COUNT(DISTINCT tot_rep_response_date) AS tot_rep_total_dates,
...
total_response_month AS (
    SELECT
        ...
        COUNT(DISTINCT mth_tot_rep_response_date) AS mth_tot_rep_total_dates,
...
liberator_pcn_icdate_received_last13months AS (
    SELECT 
        *,
        ...
        /*links to officer monthly totals*/
        CAST(Response_written_by || SUBSTR(response_generated_at, 1, 10) AS VARCHAR) AS link_officer_total_rep_dates,
        CAST(Response_written_by || IF(response_generated_at <> '', SUBSTR(response_generated_at, 1, 7) || '-01', '') AS VARCHAR) AS link_month_total--,
...
liberator_with_team_gds AS (
    SELECT
        ...
        CAST(CASE
            WHEN l.whenassigned <> '' AND l.response_generated_at = '' 
        ...
        SUBSTR(CAST(l.date_received AS VARCHAR(10)), 1, 7) || '-01' AS MonthYear, -- asserted date_received <>''
        ...
        */
        ...
        /*links to officer monthly totals*/
        l.link_officer_total_rep_dates,
        l.link_month_total,

        /* officer monthly totals*/
        CAST(m.mth_tot_rep_monthYear_response AS VARCHAR) AS mth_tot_rep_monthYear_response,
        CAST(m.mth_tot_rep_total_dates AS VARCHAR) AS mth_tot_rep_total_dates,
        CAST(m.mth_tot_rep_records AS VARCHAR) AS mth_tot_rep_records,
        CAST(o.tot_rep_response_date AS VARCHAR) AS tot_rep_response_date,
        CAST(o.tot_rep_total_dates AS VARCHAR) AS tot_rep_total_date,
        CAST(o.tot_rep_records AS VARCHAR) AS tot_rep_records,
    ...
    */ 
    ...
    LEFT JOIN officer_total_rep_dates o
        ON l.link_officer_total_rep_dates = o.tot_rep_unique_link

    LEFT JOIN total_response_month m
        ON l.link_month_total = m.mth_tot_rep_unique_link
),
...
google_form AS (
    ...
    SELECT
        ...
        */
        ...
        -- Warning: A small number outlier cases, producing negative values in the original Glue output are also produced here...
        ...
        */
        ...
        /* Instead here, the null or empty translations are deliberately dependent upon the target datatype... 
        ...
        */
        ...
        /*officer monthly totals*/
        CAST('' AS VARCHAR) AS link_officer_total_rep_dates,
        CAST('' AS VARCHAR) AS link_month_total,
        CAST('' AS VARCHAR) AS mth_tot_rep_monthYear_response,
        CAST('' AS VARCHAR) AS mth_tot_rep_total_dates,
        CAST('' AS VARCHAR) AS mth_tot_rep_records,
        CAST('' AS VARCHAR) AS tot_rep_response_date,
        CAST('' AS VARCHAR) AS tot_rep_total_date,
        CAST('' AS VARCHAR) AS tot_rep_records,
...
SELECT * FROM liberator_with_team_gds
UNION
