...
TotlNoOfWarrants as (
      Select EA, monthyear, count(*) TotalWarrants
      from BailiffReturn
      group by EA, monthyear
      order by EA, monthyear),

BailiffEAWarrantTotals as (
      Select A.EA, A.monthyear, Status_Code, Status_description, count(*) TotNoOfWarrants
      from BailiffReturn as A
      left join TotlNoOfWarrants as B ON A.EA = B.EA AND A.MonthYear = B.MonthYear
      group by A.EA, A.monthyear, Status_Code, Status_description
      order by A.EA, A.monthyear, Status_Code, Status_description)

/*** Output Data ***/
SELECT A.*, TotalWarrants, 
       CAST((CAST(TotNoOfWarrants as double)/TotalWarrants) * 100 as decimal(11,2)) as Percentage_Status_Code,
...
FROM BailiffEAWarrantTotals as A
LEFT JOIN TotlNoOfWarrants as B ON A.EA = B.EA AND A.MonthYear = B.MonthYear
ORDER BY A.EA, A.monthyear, Status_Code
...
S3bucket_node3 = glueContext.getSink(
    path="s3://dataplatform-" + environment + "-refined-zone/parking/liberator/Parking_Bailiff_EA_Warrant_Totals/",
    connection_type="s3",
...
S3bucket_node3.setCatalogInfo(
    ...
    catalogTableName="Parking_Bailiff_EA_Warrant_Totals",
)
