...
if __name__ == "__main__":
    ...
    df2 = df2.drop("import_datetime","import_timestamp","import_year","import_year","import_month","import_day","import_date")
    ...
    #output = output.withColumnRenamed("created","created_date")
    
    #convert the coordinates
    output = convert_bng_to_latlon(output,'easting','northing')
    
    # create output to gecode
    
    geo = output.withColumnRenamed("lat","latitude")
    geo = geo.withColumnRenamed("lon","longitude")
    geo = geo.select("complaint_id","latitude","longitude","import_year", "import_month", "import_day", "import_date") 
    geo = geo.filter(geo.latitude.isNotNull())
    geo = geo.withColumnRenamed("complaint_id","id")
    geo = geo.withColumn("Source", lit('noiseworks'))
