...
def main():
    ...
    ####################### GET VARIABLES FROM GLUE #######################

    s3_bucket = get_glue_env_var('s3_bucket', '')
    output_folder_name = get_glue_env_var('output_folder_name', '')
    api_to_call = get_glue_env_var('api_to_call', '')
    table_to_call = get_glue_env_var('table_to_call', '')
    ...
    if(len(dates_to_call) > 30):
        print(f'{len(dates_to_call)} Dates to Call. Trimming to a 30 Dates')
        dates_to_call = dates_to_call[:30]
