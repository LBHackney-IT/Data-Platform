...
def get_latest_snapshot(dfa, snapshot_date_col):
    ...
    """
    dfa = dfa.where(col(snapshot_date_col) == dfa.select(
        max(snapshot_date_col)).first()[0])
    return dfa
...
def set_increment_df(source_catalog_database, increment_table_name,
                     increment_date_col, id_col, last_snapshot_date='1970-01-01'
                     ):
    """
    ...
    df = prepare_increments(
        df, id_col, increment_date_col)
...
if __name__ == "__main__":
    ...
    snapshot_catalog_database = get_glue_env_var(
        'snapshot_catalog_database', '')
    increment_catalog_database = get_glue_env_var(
    ...
    if not (
        table_exists_in_catalog(glueContext, snapshot_table_name,
                                snapshot_catalog_database)
        or table_exists_in_catalog(glueContext, increment_table_name,
                                   increment_catalog_database)
    ):
    ...
    if not table_exists_in_catalog(glueContext, snapshot_table_name, snapshot_catalog_database):
        increment_df = set_increment_df(increment_catalog_database, increment_table_name,
                                        increment_date_col, id_col)
        ...
        increment_df = set_increment_df(
            glueContext, increment_catalog_database, increment_table_name,
            last_snapshot_date
        )
