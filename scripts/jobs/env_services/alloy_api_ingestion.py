...
def download_file_to_df(file_id, api_key, filename):
    """
    download export from api, extract csv from exported zipfile and read csv to pandas df
    """
...
def get_last_import_date_time(glue_context, database, resource):
    """
    get latest import date from existing data in AWS, if no data found return origin date
    """
...
def format_time(date_time):
    """
    format date time from to expected format used in API request payload
    """
...
def update_aqs(alloy_query, last_import_date_time):
    """
    update alloy query with last import date to retrieve only changed items
    """
...
def get_task_id(response):
    """
    return task id from api response
    """
...
def get_task_status(response):
    """
    return task status from api response
    """
    if response.status_code != 200:
        logger.info(
...
def get_file_item_id(response):
    """
    return item id for file from api response
    """
    if response.status_code != 200:
        logger.info(
