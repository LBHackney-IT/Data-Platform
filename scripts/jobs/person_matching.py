...
def standardize_name(name: Column) -> Column:
    ...
    * Remove following characters romt the start of the name: ampersand (&), asterisk(*), dot(.), any slash(\\\ or /)
...
def prepare_clean_council_tax_data(spark: SparkSession, council_tax_account_data_path: str,
    ...
    Args:
        ...
        council_tax_non_liability_person_data_path: Path of the S3 (or local) folder containing council tax non-liability data
...
def prepare_clean_housing_benefit_data(spark: SparkSession, housing_benefit_member_data_path: str,
    ...
    Args:
        ...
        housing_benefit_rent_assessment_data_path: Path of the S3 (or local) folder containing housing benefit rent assessment data
        housing_benefit_tax_calc_stmt_data_path: Path of the S3 (or local) folder containing housing benefit tax calculation statement data
...
def remove_deceased(df: DataFrame) -> DataFrame:
    """Remove deceased person from the DataFrame. For the data sources we have cleansed this information was present
    along with other details of the person for e.g. if the person name is given Executors of Mr. Abc Pqr Xyz then
    title is Executors of Mr. while first name is Abc, middle name Pqr and Xyz as last name. Hence deceased information
...
def generate_possible_matches(df: DataFrame) -> DataFrame:
    ...
    Returns:
        A DataFrame that contains all the possible match with columns starting with 'a_' and 'b_' to represent two person a and b that we comparing.
...
def generate_features(input_df: pd.DataFrame) -> pd.DataFrame:
    ...
    * b_full_address: Full address from the 'b' dataset
    ...
    Returns:
        A Spark struct/pandas dataframe containing following columns first_name_similar, middle_name_similar, last_name_similar, name_similarity, address_line_1_similarity, address_line_2_similarity, full_address_similarity
...
def feature_engineering(df: DataFrame) -> DataFrame:
    ...
    * uprn_same: Matches UPRN. Possible values: match (for perfect match), non-match (for mismatch), unknown (if either value is missing)
    * title_same: Matches title. Possible values: match (for perfect match), non-match (for mismatch), unknown (if either value is missing)
    * date_of_birth_same: Matches date of birth. Possible values: match (for perfect match), non-match (for mismatch), unknown (if either value is missing)
    * date_of_birth_same: Matches date of birth. Possible values: match (for perfect match), non-match (for mismatch), unknown (if either value is missing)
...
def link_all_matched_persons(standard_df: DataFrame, predicted_df: DataFrame) -> DataFrame:
    ...
    Returns:
        A DataFrame similar to standard DataFrame but conatining a column "nmatching_id" to indicate similar persons
    ...
    # Graph query using motif to find where a is connected to b and b is also connected to a
...
def main():
    ...
    # arguments for housing data
    parser.add_argument("--person_reshape_data_path", type=str, required=True,
                        metavar=f"set --housing_benefit_data_path=path to person reshape dataset")
    parser.add_argument("--assets_reshape_data_path", type=str, required=True,
                        metavar="set --assets_reshape_data_path=path to assets reshape dataset")
    parser.add_argument("--tenure_reshape_data_path", type=str, required=True,
                        metavar="set --tenure_reshape_data_path=path to tenure reshape dataset")

    # arguments for council tax data
    parser.add_argument("--council_tax_account_data_path", type=str, required=True,
                        metavar="set --council_tax_account_data_path=path to council tax account dataset")
    parser.add_argument("--council_tax_liability_person_data_path", type=str, required=True,
                        metavar="set --council_tax_liability_person_data_path=path to council tax liability person "
                                "dataset")
    parser.add_argument("--council_tax_non_liability_person_data_path", type=str, required=True,
                        metavar="set --council_tax_non_liability_person_data_path=path to council tax non liability "
                                "person dataset")
    parser.add_argument("--council_tax_occupation_data_path", type=str, required=True,
                        metavar="set --council_tax_occupation_data_path=path to council tax occupation dataset")
    parser.add_argument("--council_tax_property_data_path", type=str, required=True,
                        metavar="set --council_tax_property_data_path=path to council tax property dataset")

    # arguments for housing benefit data
    parser.add_argument("--housing_benefit_member_data_path", type=str, required=True,
                        metavar="set --housing_benefit_member_data_path=path to housing benefit member dataset")
    parser.add_argument("--housing_benefit_household_data_path", type=str, required=True,
                        metavar="set --housing_benefit_household_data_path=path to housing benefit household dataset")
    parser.add_argument("--housing_benefit_claim_data_path", type=str, required=True,
                        metavar="set --housing_benefit_claim_data_path=path to housing benefit claim dataset")
    parser.add_argument("--housing_benefit_tax_claim_data_path", type=str, required=True,
                        metavar="set --housing_benefit_tax_claim_data_path=path to housing benefit tax claim dataset")
    parser.add_argument("--housing_benefit_rent_assessment_data_path", type=str, required=True,
                        metavar="set --housing_benefit_rent_assessment_data_path=path to housing benefit rent "
                                "assessment dataset")
    parser.add_argument("--housing_benefit_rent_claim_data_path", type=str, required=True,
                        metavar="set --housing_benefit_rent_claim_data_path=path to housing benefit rent claim dataset")
    parser.add_argument("--housing_benefit_tax_calc_stmt_data_path", type=str, required=True,
                        metavar="set --housing_benefit_tax_calc_stmt_data_path=path to housing benefit tax calc "
                                "statement")

    # arguments for parking permit data
    parser.add_argument("--parking_permit_data_path", type=str, required=True,
                        metavar="set --parking_permit_data_path=path to parking permit dataset")

    parser.add_argument(f"--output_path", type=str, required=False,
                        metavar=f"set --output=path to write the result")

    person_reshape_data_path_arg = "person_reshape_data_path"
    assets_reshape_data_path_arg = "assets_reshape_data_path"
    tenure_reshape_data_path_arg = "tenure_reshape_data_path"
    council_tax_account_data_path_arg = "council_tax_account_data_path"
    council_tax_liability_person_data_path_arg = "council_tax_liability_person_data_path"
    council_tax_non_liability_person_data_path_arg = "council_tax_non_liability_person_data_path"
    council_tax_occupation_data_path_arg = "council_tax_occupation_data_path"
    council_tax_property_data_path_arg = "council_tax_property_data_path"
    housing_benefit_member_data_path_arg = "housing_benefit_member_data_path"
    housing_benefit_household_data_path_arg = "housing_benefit_household_data_path"
    housing_benefit_rent_assessment_data_path_arg = "housing_benefit_rent_assessment_data_path"
    housing_benefit_tax_calc_stmt_data_path_args = "housing_benefit_tax_calc_stmt_data_path"
    parking_permit_data_path_arg = "parking_permit_data_path"

    output_path_arg = "output_path"
    glue_args = [person_reshape_data_path_arg, assets_reshape_data_path_arg, tenure_reshape_data_path_arg,
                 council_tax_account_data_path_arg, council_tax_liability_person_data_path_arg,
                 council_tax_non_liability_person_data_path_arg, council_tax_occupation_data_path_arg,
                 council_tax_property_data_path_arg,
                 housing_benefit_member_data_path_arg, housing_benefit_household_data_path_arg,
                 housing_benefit_tax_calc_stmt_data_path_args,
                 parking_permit_data_path_arg,
                 output_path_arg]
    ...
    with ExecutionContextProvider(mode, glue_args, local_args) as execution_context:
        ...
        filter_deceased = True
        prepare_training_data = False
        training_required = False

        person_reshape_data_path = execution_context.get_input_args(person_reshape_data_path_arg)
        assets_reshape_data_path = execution_context.get_input_args(assets_reshape_data_path_arg)
        tenure_reshape_data_path = execution_context.get_input_args(tenure_reshape_data_path_arg)

        council_tax_account_data_path = execution_context.get_input_args(council_tax_account_data_path_arg)
        council_tax_liability_person_data_path = execution_context.get_input_args(
            council_tax_liability_person_data_path_arg)
        council_tax_non_liability_person_data_path = execution_context.get_input_args(
            council_tax_non_liability_person_data_path_arg)
        council_tax_occupation_data_path = execution_context.get_input_args(council_tax_occupation_data_path_arg)
        council_tax_property_data_path = execution_context.get_input_args(council_tax_property_data_path_arg)

        housing_benefit_member_data_path = execution_context.get_input_args(housing_benefit_member_data_path_arg)
        housing_benefit_household_data_path = execution_context.get_input_args(housing_benefit_household_data_path_arg)
        housing_benefit_rent_assessment_data_path = execution_context. \
            get_input_args(housing_benefit_rent_assessment_data_path_arg)
        housing_benefit_tax_calc_stmt_data_path = execution_context \
            .get_input_args(housing_benefit_tax_calc_stmt_data_path_args)

        parking_permit_data_path = execution_context.get_input_args(parking_permit_data_path_arg)
        ...
        standard_output_path = f"{output_path}/standard"
        auto_labeled_data_path = f"{output_path}/auto_labeled"
        feature_engineering_data_path = f"{output_path}/feature_engineering"
        manually_labelled_data_path = f"{output_path}/manually_labelled"
        model_path = f"{output_path}/trained_model"

        if prepare_training_data:
            housing_cleaned = prepare_clean_housing_data(spark, person_reshape_data_path, assets_reshape_data_path,
                                                         tenure_reshape_data_path)
            housing_deceased_removed = remove_deceased(housing_cleaned) if filter_deceased else housing_cleaned
            housing = standardize_housing_data(housing_deceased_removed)

            council_tax_cleaned = prepare_clean_council_tax_data(spark, council_tax_account_data_path,
                                                                 council_tax_liability_person_data_path,
                                                                 council_tax_non_liability_person_data_path,
                                                                 council_tax_occupation_data_path,
                                                                 council_tax_property_data_path)
            council_tax_deceased_removed = remove_deceased(
                council_tax_cleaned) if filter_deceased else council_tax_cleaned
            council_tax = standardize_council_tax_data(council_tax_deceased_removed)

            housing_benefit_cleaned = prepare_clean_housing_benefit_data(spark,
                                                                         housing_benefit_member_data_path,
                                                                         housing_benefit_household_data_path,
                                                                         housing_benefit_rent_assessment_data_path,
                                                                         housing_benefit_tax_calc_stmt_data_path)
            housing_benefit_deceased_removed = remove_deceased(
                housing_benefit_cleaned) if filter_deceased else housing_benefit_cleaned
            housing_benefit = standardize_housing_benefit_data(housing_benefit_deceased_removed)

            parking_permit_cleaned = prepare_clean_parking_permit_data(spark, parking_permit_data_path)
            parking_permit_deceased_removed = remove_deceased(
                parking_permit_cleaned) if filter_deceased else parking_permit_cleaned
            parking_permit = standardize_parking_permit_data(parking_permit_deceased_removed)

            standard_df = housing.union(council_tax).union(housing_benefit).union(parking_permit).coalesce(5)
            standard_df.write.parquet(standard_output_path)
            logger.info(f"Standardized data written successfully to {standard_output_path}")

            standard_df = spark.read.parquet(standard_output_path)
            possible_matches = generate_possible_matches(standard_df)
            auto_labeled_data = automatically_label_data(possible_matches)

            auto_labeled_data.write.parquet(auto_labeled_data_path)
            logger.info(f"Auto labeled data written successfully to {auto_labeled_data_path}")

            feature_engineering_df = feature_engineering(auto_labeled_data)
            feature_engineering_df.write.parquet(feature_engineering_data_path)
            logger.info(f"Feature engineering data written successfully to {feature_engineering_data_path}")

        if training_required:
            feature_engineering_df = spark.read.parquet(feature_engineering_data_path)
            manually_labelled_df = spark.read \
                .csv(header=True, inferSchema=True, path=manually_labelled_data_path) \
                .select(col("a_source"), col("a_source_id"), col("b_source"), col("b_source_id"),
                        trim(lower(col("manual_label"))).alias("manual_label"), col("confidence_score")) \
                .filter(col("manual_label").isin(["match", "non-match"]))

            labeled_df = feature_engineering_df \
                .join(manually_labelled_df, ["a_source", "a_source_id", "b_source", "b_source_id"]) \
                .withColumn("label",
                            when((col("auto_labels") == lit(True)) | (col("manual_label") == lit("match")), lit(1.0))
                            .otherwise(lit(0.0))) \
                .withColumn("label_confidence_score",
                            when(col("auto_labels") == lit(True), lit(1.0))
                            .otherwise(col("confidence_score") / 5.0)) \
                .drop(*["auto_labels", "manual_label", "confidence_score"]) \
                .filter(col("label_confidence_score").isNotNull())

            train_model(labeled_df, model_path)

        # Complete flow for any new data. Please use below as an example.
        # Let's assume we want to find all duplicates person in parking dataset and parking data is a new data.
        new_data_path = parking_permit_data_path  # Replace parking_permit_data_path with the path of your data source
        # Replace clean_parking_permit_data() with method relevant for your data
        new_data_cleaned = prepare_clean_parking_permit_data(spark, new_data_path)
        # For most of the cases we don't want to compare deceased persons
        new_data_deceased_removed = remove_deceased(new_data_cleaned) if filter_deceased else new_data_cleaned
        # Replace standardize_parking_permit_data() with method relevant for your data
        new_data_standard = standardize_parking_permit_data(new_data_deceased_removed)
        new_data_result = match_persons(model_path, new_data_standard)  # No change required
        new_data_result.show(n=1000, truncate=False)
        # Often you may want to save it as CSV to give the result to a human
        new_data_result.write.csv(header=True, path=f"{output_path}/new_data_matched_csv")
        # If you want the result to be used by another Glue Job or any reporting tool
        new_data_result.write.parquet(f"{output_path}/new_data_matched_parquet")
