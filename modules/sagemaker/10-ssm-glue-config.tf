...
resource "aws_ssm_parameter" "dev_enpoint_config" {
  tags = var.tags

  name        = "/${var.identifier_prefix}glue-dev-endpoint/${var.instance_name}"
  type        = "String"
  description = "The configuration for the glue development endpoint"
  value       = jsonencode(local.glue_dev_endpoint_config)
}
