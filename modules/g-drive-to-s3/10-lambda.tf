...
resource "aws_lambda_function" "g_drive_to_s3_copier_lambda" {
  ...
  runtime          = "python3.8"
  function_name    = "${var.identifier_prefix}-g-drive-to-s3-copier-for-${var.lambda_name}-glue-job"
...
resource "aws_cloudwatch_event_target" "check_foo_every_day_at_10" {
  rule      = aws_cloudwatch_event_rule.every_day_at_10.name
