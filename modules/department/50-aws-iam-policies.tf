...
data "aws_iam_policy_document" "secrets_manager_read_only" {
  statement {
    ...
    resources = [
      ...
      "arn:aws:secretsmanager:eu-west-2:${data.aws_caller_identity.current.account_id}:secret:${var.identifier_prefix}${local.department_identifier}-sheets-credential-*"
    ]
  }
